...
```python
# 获取会话信息
response = requests.get("http://localhost:8000/session/session123")
session_info = response.json()
print(f"会话状态: {session_info}")

# 关闭会话
response = requests.post("http://localhost:8000/session/session123/close")
...
```python
# 获取用户统计
response = requests.get("http://localhost:8000/user/user123/stats")
user_stats = response.json()
print(f"用户统计: {user_stats}")

# 获取记忆统计
response = requests.get("http://localhost:8000/user/user123/memories")
